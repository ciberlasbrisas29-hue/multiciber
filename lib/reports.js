...
export async function getAdvancedReportData(userId, period = 'today') {
  ...
  const businessSettings = await BusinessSettings.findOne({ userId: new mongoose.Types.ObjectId(userId) }).lean();
  const businessLogo = businessSettings?.businessLogo || null;
  const businessName = businessSettings?.businessName || null;
