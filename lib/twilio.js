...
export async function sendDebtReminder(phoneNumber, debt) {
  ...
  const debtAmount = debt.debtAmount || (debt.total - (debt.paidAmount || 0));
  
  const message = `📋 *RECORDATORIO DE DEUDA*\n\n` +
    `Hola ${debt.clientName || 'Cliente'},\n\n` +
    `Tienes una deuda pendiente:\n\n` +
    `*Concepto:* ${debt.concept || 'Venta'}\n` +
    `*Número de venta:* ${debt.saleNumber || 'N/A'}\n` +
    `*Total de la venta:* $${debt.total?.toFixed(2) || '0.00'}\n` +
    `*Monto pagado:* $${(debt.paidAmount || 0).toFixed(2)}\n` +
...
/**
 * Envía recordatorios de deuda a todos los clientes que tienen deudas pendientes
 * @param {string} userId - ID del usuario
 * @returns {Promise<Object>} - Resultado del envío
 */
export async function sendDebtRemindersToAllClients(userId) {
  if (!isTwilioConfigured()) {
    logger.warn('Twilio no está configurado. No se pueden enviar recordatorios de deuda.');
    return {
      success: false,
      message: 'Twilio no está configurado',
      error: 'TWILIO_NOT_CONFIGURED'
  ...
  try {
    ...
    const allDebts = await Sale.find({
      createdBy: new mongoose.Types.ObjectId(userId),
      status: 'debt',
    ...
    for (const client of clients) {
      ...
      // Construir mensaje con todas las deudas
      let message = `📋 *RECORDATORIO DE DEUDA*\n\n`;
      message += `Hola ${client.name},\n\n`;
      message += `Tienes ${clientDebts.length} deuda${clientDebts.length > 1 ? 's' : ''} pendiente${clientDebts.length > 1 ? 's' : ''}:\n\n`;

      // Agregar cada deuda
      clientDebts.forEach((debt, index) => {
        const debtAmount = debt.debtAmount || (debt.total - (debt.paidAmount || 0));
        if (debtAmount > 0) {
          message += `*${index + 1}. ${debt.concept || 'Venta'}*\n`;
          message += `   Número: ${debt.saleNumber || 'N/A'}\n`;
          message += `   Total: $${debt.total?.toFixed(2) || '0.00'}\n`;
          message += `   Pagado: $${(debt.paidAmount || 0).toFixed(2)}\n`;
          message += `   *Restante: $${debtAmount.toFixed(2)}*\n\n`;
        }
      });

      message += `*Total pendiente: $${totalDebt.toFixed(2)}*\n\n`;
      message += `Por favor, acércate a realizar el pago. ¡Gracias!`;
      ...
      try {
        ...
        logger.error('Error enviando recordatorio de deuda a cliente', {
          client: client.name,
          phone: client.phone,
          error: error.message
        });
    ...
    return {
      success: true,
      message: `Se enviaron ${sent} recordatorios exitosamente. ${failed} fallaron.`,
      sent,
      failed,
      results
    };
  } catch (error) {
    logger.error('Error enviando recordatorios de deuda a todos los clientes:', error);
    return {
      success: false,
...
export async function sendDailyReport(phoneNumber, period = 'today', reportData = null) {
  try {
    ...
    let userId = reportData.userId || 'default';
    ...
    // Usar encodeURIComponent para asegurar que el userId se codifique correctamente
    const pdfPath = `/api/reports/pdf/${reportId}?period=${period}&userId=${encodeURIComponent(userId)}`;
    const pdfUrl = `${baseUrl}${pdfPath}`;
    
    logger.info('URL del PDF construida', {
      pdfUrl,
      baseUrl,
      reportId,
      userId,
    ...
    const message = `📊 *Reporte - ${periodLabel}*\n\nAquí está tu reporte en formato PDF.`;
