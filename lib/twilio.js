...
async function sendWhatsAppMedia(to, message, mediaUrl) {
  ...
  try {
    ...
    return {
      success: false,
      message: error.message || 'Error al enviar mensaje multimedia',
...
export async function sendDailyReport(phoneNumber, period = 'today', reportData = null) {
  try {
    ...
    // Obtener userId desde reportData si está disponible, o usar un valor por defecto
    // NOTA: Necesitamos pasar userId en la URL para que el endpoint pueda generar el reporte correcto
    // En producción, deberías usar un sistema de tokens temporales más seguro
    ...
    // NOTA: Para que esto funcione en producción, el servidor debe ser accesible públicamente
    // Si estás usando Vercel, Netlify, o similar, esto funcionará automáticamente
    // Si estás usando un servidor local, necesitarás usar ngrok o similar para exponerlo
